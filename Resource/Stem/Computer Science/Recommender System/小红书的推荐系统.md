...
1. 召回：从数据库中快速取回一些笔记，例如调用几十条召回通道，每条召回通道取回几十到几百个笔记，共取回几千篇笔记
2. 粗排：选出用户最感兴趣的一些笔记，规模比较小的机器学习模型给几千篇笔记打分，按照分数做排序和截断，保留分数最高的几百篇笔记
3. 精排：规模比较大的神经网络给几百篇笔记逐一打分，评估用户的兴趣
4. 重排：根据精排分数和多样性分数做随机抽样，得到几十篇笔记，并把相似内容打散、插入广告和运营内容
...
```json
{
	...
	"elements": [
		...
		},
		{
			...
			"seed": 1117385450,
			"version": 3,
			"versionNonce": 913711914,
			"isDeleted": true,
			"boundElements": null,
			"updated": 1778813794724,
			"locked": false,
			"text": "",
			"rawText": "",
			"fontSize": 20,
